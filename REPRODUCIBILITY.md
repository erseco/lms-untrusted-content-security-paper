...
| `resultados-h5p-echo-video.json` | Registro no operacional de la *stored DOM XSS* `H5P.Video`/EchoVideo: cadena de código, microprueba pasiva Chromium y ejecución privada *end-to-end* en Moodle y WordPress confirmadas, sin publicar el valor de entrada; incluye rol/capacidades, versiones y topología de la prueba WordPress. |
...
| `resultados-label-xss.json` | **Efectos del contenido activo de Etiquetas** (`lab/run-label-check.sh` + `label-xss-test.cjs`): dentro del modelo documentado `RISK_XSS`, un Profesor con edición crea una Etiqueta (`mod_label`) con `<script>`/`<img onerror>`; al cargar la página del curso **se ejecutan** (`scriptExecuted: true`) en la ventana superior, *same-origin* — mismo `noclean=true` que `mod_page`, vía `format_module_intro` (`weblib.php:872`). No se clasifica como evasión de saneamiento ni *0-day*. |
...
Se registra por separado una **excepción concreta de parámetros** en
...
La evidencia pública contiene solo la cadena y booleanos pasivos; no incluye el valor
de entrada ni un paquete. La ejecución real se confirmó con la sonda común en Moodle
y WordPress. En WordPress 7.0.2 + H5P 1.17.8, un `Author` con
