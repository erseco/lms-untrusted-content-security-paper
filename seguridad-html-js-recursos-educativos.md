...
|---|---|---|
| `mod_page` ejecuta `<script>` de autor | Vivo (Moodle 5.0.7 local) + código | [4.2](#sec-4-2); `mod/page/view.php:90-93` |
| SCORM nativo se ejecuta *same-origin* sin sandbox | Vivo + código | [4.3](#sec-4-3); `player.php:279-285` |
| H5P filtra los *parámetros* (no ejecutan) | Vivo (control negativo) | [4.4](#sec-4-4); `poc/evil.h5p` |
...
| El modo seguro aísla (origen opaco) | Vivo en **prototipo** (modificación de código propuesta; Chromium y Firefox/Gecko, vía Playwright) | [4.5](#sec-4-5)–[4.6](#sec-4-6); `resultados-firefox*.json` |
...
| `mod_exelearning`, `wp-exelearning`, `omeka-s-exelearning` | Autor/mantenedor | Objeto evaluado (estable) **y** mitigación propuesta (modo seguro, 6.2.2; propuesta de modificación de código) | `legacy`: vivo · `secure`: vivo en prototipo (Chromium + Firefox/Gecko, vía Playwright) |
