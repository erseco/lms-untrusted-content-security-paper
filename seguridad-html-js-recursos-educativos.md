...
H5P es un caso aparte: no ejecuta HTML de autor, sino librerías curadas. Sobre `mod_page` precisamos un punto que la literatura suele confundir: su protección real no es el saneamiento *server-side* (usa `noclean=true`), sino la restricción de capacidad/rol (`mod/page:addinstance`).
...
|---|---|---|---|---|
| `mod_page` (Página) | Sí (`noclean=true`; verificado) | Sí (ventana superior, sin iframe) | Ninguno *server-side*; solo restringido por la capacidad `mod/page:addinstance` | Alto si lo edita el profesorado (se ejecuta en la sesión de cada visitante) |
...
| El modo seguro aísla (origen opaco) | En ejecución sobre el **prototipo** (modificación de código propuesta; Chromium y Firefox/Gecko, vía Playwright); reconfirmado de forma **transversal en Moodle 4.5, 5.0, 5.1 y 5.2** | [4.5](#sec-4-5)–[4.6](#sec-4-6); `resultados-firefox*.json`, `evidencias/resultados-matriz-versiones.json` |
...
#### 6.2.2 Prototipo de referencia (modificación de código propuesta) en las integraciones de eXeLearning {#sec-6-2-2}
...
| Otros visitantes | propagación por mensaje (mensajería AJAX, rol `user`) | dependiente de interacción | contenido confinado al origen; revisión por rol |
...
Entorno local y desechable (no medimos producción), con una excepción: una única confirmación autorizada y reversible sobre una instancia en línea (sección 4.2) —un laboratorio de pruebas operado por el equipo de desarrollo de eXeLearning— realizada sobre cuentas propias o de laboratorio y revertida, coherente con la declaración ética (sección 8); versiones concretas (los resultados se atan a los *commits* de la sección 3.1); sin explotación destructiva (demostramos la cadena, no el abuso); verificado en **dos motores** (Chromium y Firefox/Gecko, vía Playwright), con **Safari/WebKit no probado** (trabajo futuro); la reverificación en ejecución sobre el prototipo (origen opaco, la cabecera de respuesta R3) y el test adversarial del puente se ejecutaron con **Chromium** sobre el prototipo (*commit* `73fe6ff`), por lo que una **reejecución del test del puente en Firefox** y Safari/WebKit quedan como trabajo futuro (la evidencia de aislamiento entre motores en Firefox no cambia); el vector de librería H5P está verificado en código y con PoC validada, con la ejecución *end-to-end* como procedimiento manual (la automatización *headless* del selector de ficheros de Moodle 5 no fue fiable); `mod_exeweb`/`mod_exescorm` se infieren del código, no de prueba en ejecución; la IA generativa (RQ4) no se mide. El modelo de amenaza se centra en el aislamiento cliente, no en toda la superficie del LMS. La generalización a otras versiones o configuraciones exige re-verificar contra el código correspondiente.
...
| `mod_exelearning`, `wp-exelearning`, `omeka-s-exelearning` | Autor/mantenedor | Objeto evaluado (estable) **y** mitigación propuesta (modo seguro, 6.2.2; propuesta de modificación de código) | `legacy`: en ejecución · `secure`: en ejecución sobre el prototipo (*commit* `73fe6ff`) — aislamiento de origen opaco, la cabecera CSP `sandbox` de respuesta (R3) y el rechazo del puente a un emisor hostil (7/7, Chromium) verificados en ejecución, además de la evidencia de aislamiento existente en Firefox/Gecko (vía Playwright) |
| Safari / WebKit | — | Cobertura de navegador | Pendiente (trabajo futuro) |

Sobre el prototipo (*commit* `73fe6ff`, Chromium) verificamos además en ejecución tres propiedades del modo seguro en este *checkout*: el aislamiento de origen opaco (leer `parent.document` / `parent.M.cfg.sesskey` lanza `SecurityError` en ambos casos), la cabecera CSP `sandbox` de respuesta (R3) y el rechazo del puente SCORM a un emisor hostil dentro del iframe (7/7 casos adversariales) —además de la evidencia de aislamiento entre motores existente en Firefox/Gecko, que no cambia; una reejecución del test del puente en Firefox y Safari/WebKit quedan como trabajo futuro—. Las afirmaciones sobre software propio se sostienen sobre artefactos publicados y reejecutables de forma independiente (`archivo:línea`, JSON de evidencia, PoC), no sobre la mera inspección del autor, aunque todavía no han sido replicadas de forma independiente.
