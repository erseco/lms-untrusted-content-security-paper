...
| **H5P.Video · URL EchoVideo** | **Sí, confirmado** | Sí en el tipo `iframe` probado | iframe H5P y `about:srcdoc` sin `sandbox` | Código + microprueba Chromium + ejecución Moodle/WordPress; divulgación pendiente |
| **H5P.IFrameEmbed · documento activo** | Depende del documento | Sí si el recurso ya es *same-origin* | iframe sin `sandbox` | Composición: `content/` bloquea HTML/JS/SVG/XML por defecto; no es vía genérica de autoría por sí sola |
| **Moodle H5P · librería `div`** | **Sí** (`preloadedJs`) | Sí: documento `/h5p/embed.php` | iframe exterior sin `sandbox` | Frontera: `moodle/h5p:updatelibraries`; runtime de la sonda completa pendiente |
| **Moodle H5P · librería `iframe`** | **Sí** (`preloadedJs`) | Sí: `about:blank` heredado | iframe interior sin `sandbox`, escrito con `contentDocument.write` | Misma capacidad; runtime pendiente |
| **WordPress H5P · librería `div`** | **Sí** (`preloadedJs`) | Sí: documento WordPress superior | — (sin iframe de contenido) | Frontera: `manage_h5p_libraries`; runtime pendiente |
| **WordPress H5P · librería `iframe`** | **Sí** (`preloadedJs`) | Sí: `about:blank` heredado | iframe interior sin `sandbox` | Misma capacidad; runtime pendiente |
| **WordPress H5P · embed externo** | Sí (en el origen WordPress) | No respecto al sitio consumidor cross-origin | depende del consumidor | SOP separa el padre consumidor; revisar cookies/contexto del endpoint por separado |
...
| H5P · parámetros genéricos | No en control publicado | Sí | — | cualquiera | n/a | No | n/a | control negativo (en ejecución) | no prueba cada consumidor/sink |
| H5P.Video · EchoVideo | Sí, confirmado | Sí (`iframe`) | autor ordinario si la librería vulnerable está instalada | cualquiera | no localizado en la vista WP probada | No | esperado según rol visitante | código + microprueba Chromium + ejecución Moodle/WordPress como `Author` | *stored DOM XSS* *same-origin* |
| Moodle H5P · `div` | Sí (`preloadedJs`) | Sí | manager (cap. H5P) | cualquiera | esperado: sí | No | Sí | artefacto reproducible; runtime pendiente | JS arbitrario *same-origin* |
| Moodle H5P · `iframe` | Sí (`preloadedJs`) | Sí (`about:blank`) | manager (cap. H5P) | cualquiera | esperado: sí | No | Sí | artefacto reproducible; runtime pendiente | JS arbitrario *same-origin* |
| WP H5P · `div` | Sí (`preloadedJs`) | Sí (top) | admin (cap. H5P) | cualquiera | esperado: nonce REST | No | Sí | artefacto reproducible; runtime pendiente | JS arbitrario en WordPress |
| WP H5P · `iframe` | Sí (`preloadedJs`) | Sí (`about:blank`) | admin (cap. H5P) | cualquiera | esperado: nonce REST | No | Sí | artefacto reproducible; runtime pendiente | JS arbitrario *same-origin* |
| `mod_exe` · estable | Sí | Sí | profesor | cualquiera | Sí | No | Sí | lee `sesskey`; mutación acotada por rol (en ejecución) | escalado por rol |
| `mod_exe` · secure | Sí | No (opaco) | profesor | cualquiera | No (token solo-lectura) | Parcial (base) | solo vía puente | error SOP (3 motores) | aislado |
| `exeweb` / `exescorm` | Sí | Sí | profesor | cualquiera | Sí | No | Sí | lee DOM/cookie; alcanza API SCORM 1.2 | acceso total *same-origin* |
| WP/Omeka eXe · estable | Sí | Sí | autor/editor | cualquiera | Sí | No | Sí | acceso al padre (en ejecución) | escalado por rol |
| WP/Omeka eXe · secure | Sí | No (opaco) | autor/editor | cualquiera | No | Parcial (base) | — | opaco (3 motores) | aislado |

*CSP restr. = «Parcial (base; sin perfil estricto)»: el modo seguro preserva el origen opaco mediante la directiva `sandbox` **emitida a nivel de respuesta**. La ejecución confirmó esa directiva (resultado live secure admin, commit `73fe6ff`). Además, `connect-src 'self'` **cierra el canal principal de exfiltración**, sin `fetch`, XHR ni `sendBeacon` a terceros; véase el método `content_security_policy()` del reproductor. El residual es un *pixel* GET porque `img-src`, `media-src` y `script-src` aún admiten HTTPS. El origen opaco deja el `sesskey` fuera de alcance, por lo que ese residual solo expone el *token* de solo lectura del paquete y datos in-SCO, no la sesión. El perfil estricto que también cierra el *pixel* queda como trabajo futuro (sección 6.3 del artículo).*
...
> **Nota (alcance de la evidencia).** El veredicto más fuerte de estas dos integraciones —«acceso total *same-origin*» (riesgo Alto), reflejado en la fila de `mod_exeweb` / `mod_exescorm` de la matriz de riesgo formal §1.1— está **confirmado en ejecución** sobre Moodle 5.2.1 (`evidencias/resultados-exeweb-exescorm.json`): desde dentro del iframe *same-origin* y **sin `sandbox`** (`#exewebobject` / `#exescorm_object`, `sandbox: null`), la sonda leyó el documento, la cookie y el `sesskey` del padre; el SCORM alcanzó además la API 1.2. Esa ejecución usó los paquetes históricos anteriores. Los actuales `evil_web.zip` / `evil_scorm.zip` son exportaciones HTML5 y SCORM 1.2 reales de una misma fuente, verificadas estructuralmente, y el *harness* queda actualizado para repetir la prueba con ellas. La demo Moodle sustituye al vuelo el avatar del DOM padre y lo resalta en verde antes del cambio persistente solicitado.
...
**Veredicto (matizado):** hay que distinguir tres planos. **(1) Filtro genérico:** el control negativo publicado confirma que H5P elimina `<script>` y `on*` del campo de texto probado, pero eso no demuestra que cada librería use siempre un escape contextual correcto. **(2) Librerías:** `preloadedJs` es código de confianza; Moodle/WordPress separan la autoría ordinaria de las capacidades administrativas de instalación, y las rutas `div`/`iframe` carecen de frontera de origen/sandbox frente a su documento anfitrión. Los paquetes pasivos construidos fuerzan ambos modos; su matriz runtime sigue pendiente (`evidencias/resultados-h5p-library.json`). **(3) Vulnerabilidad concreta:** en `H5P.Video`, una URL ordinaria de vídeo se escapa en PHP, se vuelve a decodificar en el cliente, supera el selector permisivo de Echo360 y llega a un iframe construido con `innerHTML`. La primera etiqueta que contiene el patrón es 1.6.43; la fuente 1.6.73 consultada lo conserva. Código, microprueba pasiva Chromium y ejecución *end-to-end* están confirmados en Moodle y WordPress; en WordPress 7.0.2 + H5P 1.17.8, un `Author` sin `manage_h5p_libraries` ni `disable_h5p_security` almacenó el contenido usando la librería ya instalada, y el marcador desde `about:srcdoc` leyó `parent.document` y `top.document`. La divulgación coordinada sigue pendiente y no se publica el valor (`evidencias/resultados-h5p-echo-video.json`). No se confirmó una ruta oficial actual `div`, por lo que no se afirma ejecución directa en el documento superior de WordPress. `H5P.IFrameEmbed` también crea un iframe sin `sandbox`, pero `content/` bloquea HTML/JS/SVG/XML por defecto: solo compone el riesgo si ya existe un documento activo *same-origin* (o si una capacidad administrativa como `disable_h5p_security` elimina esa precondición).
