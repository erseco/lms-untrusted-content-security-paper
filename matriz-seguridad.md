...
| **H5P.Video · URL EchoVideo** | **Candidato: sí** | Sí en el tipo `iframe` confirmado | iframe H5P sin `sandbox` | Código + microprueba Chromium + ejecución Moodle; WordPress y divulgación pendientes |
...
| H5P.Video · EchoVideo | Candidato: sí | Sí (`iframe`) | autor ordinario si la librería vulnerable está instalada | cualquiera | confirmado en la cadena Moodle | No | esperado según rol visitante | código + microprueba Chromium + ejecución Moodle | DOM XSS *same-origin*; WordPress pendiente |
