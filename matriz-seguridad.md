...
| **H5P · parámetros** | **No** | n/a para el JS inyectado | según integración | Filtrado por semántica (control negativo) |
...
**Veredicto (matizado):** hay que distinguir dos planos. En los **parámetros** de contenido, H5P **no ejecuta HTML/JS de autor**: filtra el texto con `filter_xss` contra una lista cerrada de etiquetas sin `<script>` y descarta `on*` (`h5p.classes.php:4303-4384,:5033-5054`) — control negativo. En el plano de las **librerías**, en cambio, **sí**: `preloadedJs` es código de confianza. Moodle envuelve el reproductor en `/h5p/embed.php` mediante un iframe exterior sin `sandbox`; una librería `div` corre en ese documento y una librería `iframe` añade el `about:blank` interior escrito con `contentDocument.write` (`player.php`, `h5pembed.mustache`, `h5p.js:381-437`, `h5piframe.mustache:32-34`). En WordPress, el código oficial actual (`h5p/h5p-wordpress-plugin` `1f97e674`) separa `edit_h5p_contents` de `manage_h5p_libraries`; `mayUpdateLibraries()` comprueba esta última, y guarda/carga `preloadedJs` y `embedTypes` desde `library.json`. El modo `div` puede ejecutarse en el documento WordPress y `iframe` usa la ruta interior de H5P. Los paquetes `poc/h5p-probe-moodle-div.h5p` y `poc/h5p-probe-moodle-iframe.h5p` fuerzan cada modo y llevan la sonda común completa en una salida pasiva sin mutadores. **Estado honesto:** código verificado + artefactos reproducibles construidos; las cuatro ejecuciones Moodle/WordPress × `div`/`iframe` siguen pendientes (`evidencias/resultados-h5p-library.json`). El embed externo de WordPress se registra aparte: SOP puede bloquear la lectura del sitio consumidor cross-origin, sin convertir por ello la librería en aislada respecto a su propio documento WordPress.
