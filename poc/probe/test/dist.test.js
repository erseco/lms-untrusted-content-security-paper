...
const h5pBundlePath = join(here, '..', 'dist', 'probe.h5p.bundle.js');
...
describe('dist/probe.h5p.bundle.js', () => {
  const bundle = readFileSync(h5pBundlePath, 'utf8');

  it('está al día respecto a src/', () => {
    expect(bundle).toContain('sources-sha256:' + sourcesHash());
  });

  it('publica el mismo contrato de medición en una API controlable', () => {
    expect(bundle).toContain('__EXE_POC_RESULT');
