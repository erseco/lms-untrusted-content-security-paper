...
H5P is a separate case: it does not execute author HTML, only curated libraries. On `mod_page` we correct a point the literature often confuses: its real protection is not server-side sanitisation (it uses `noclean=true`) but capability/role restriction (`mod/page:addinstance`).
...
| The secure mode isolates (opaque origin) | Live on the **prototype** (proposed code change; Chromium and Firefox/Gecko, via Playwright) | [4.5](#sec-4-5)–[4.6](#sec-4-6); `resultados-firefox*.json` |
...
A local and disposable environment (we did not measure production), with one exception: a single authorised, reversible confirmation on a live instance (Section 4.2), performed on the author's own/lab accounts and reverted, consistent with the ethics statement (Section 8); specific versions (the results are tied to the commits in Section 3.1); no destructive exploitation (we demonstrate the chain, not the abuse); verified in **two engines** (Chromium and Firefox/Gecko, via Playwright), with **Safari/WebKit not tested** (future work); the prototype live re-verification (opaque origin, the R3 response header) and the adversarial bridge test were run with **Chromium** on the prototype (commit `73fe6ff`), so a **Firefox re-run of the bridge test** and Safari/WebKit remain future work (the cross-engine Firefox isolation evidence is unchanged); the H5P library vector is verified in code and with a structurally-valid PoC, with end-to-end execution as a manual procedure, not automated headless (Moodle 5's file picker was unreliable in headless mode); `mod_exeweb`/`mod_exescorm` are inferred from code, not a live test; generative AI (RQ4) is not measured. The threat model focuses on client-side isolation, not the entire LMS surface. Generalisation to other versions or configurations requires re-verification against the corresponding code.
